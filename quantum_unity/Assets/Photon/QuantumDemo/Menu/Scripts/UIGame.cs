    ...
    public class UIGame : UIScreen<UIGame>, IConnectionCallbacks
    {
        ...
        public void Update()
        {
            if (QuantumRunner.Default != null && QuantumRunner.Default.HasGameStartTimedOut)
            {
                ...
                {
                    CustomLogger.Instance.Log("Game start timed out");
        ...
        private void OnCallbackPluginDisconnect(string reason)
        {
            ...
            {
                CustomLogger.Instance.Log("Plugin Disconnect: " + reason);
                UIMain.Client.Disconnect();
            });
        }
        ...
        {
            CustomLogger.Instance.Log("UIGame onconnected");
        }

        public void OnConnectedToMaster()
        {
            CustomLogger.Instance.Log("UIGame onconnected to master");
        }
            ...
            switch (cause)
            {
                ...
                default:
                    ...
                    if (QuantumRunner.Default?.Game?.Frames.Verified != null)
                    {
                        ...
                        CustomLogger.Instance.Log($"Created frame snapshot at tick {_frameSnapshotNumber}");
                    }
                    break;
